...
final class HermesTalkCoordinator {
    private let logger = Logger(subsystem: "net.fihonline.kallisti", category: "HermesTalk")
    ...
    private static let appDisplayName =
        Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String ?? "the assistant"

    private(set) var state: State = .idle

    private let capture: TalkAudioCapture
    private let asr: any SpeechRecognizing
    private let tts: any SpeechSynthesizing & TTSServiceProtocol
    ...
    init(
        ...
        tts: any SpeechSynthesizing & TTSServiceProtocol,
    ...
    func stopListeningAndProcess() async {
        ...
        guard !canonicalText.isEmpty else {
            ...
            if toolBoundarySeen {
                let item = TranscriptItem(
                    speaker: .system,
                    text: "\(Self.appDisplayName) used tools to process your request.",
                    isPartial: false
                )
                onTranscript?(item)
                ...
                let item = TranscriptItem(
                    speaker: .system,
                    text: "\(Self.appDisplayName) didn't produce a response. Try again.",
                    isPartial: false
                )
                onTranscript?(item)
            }
