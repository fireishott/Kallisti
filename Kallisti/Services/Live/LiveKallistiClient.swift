...
@MainActor
final class LiveKallistiClient: KallistiClientProtocol {
    private static let logger = Logger(subsystem: "net.fihonline.kallisti", category: "LiveKallistiClient")
    ...
    func ensureConversation(id: UUID) async -> EnsureResult {
        ...
        do {
            ...
            // T1.6: decode the canonical id when the server redirected
            // from a v5 alias.
            var canonicalID: UUID? = nil
            if let cidStr = response.conversationId,
               let cid = UUID(uuidString: cidStr),
               cid != id {
                canonicalID = cid
            }
            Self.logger.info("ensureConversation: sessionId=\(response.sessionId ?? "none"), created=\(response.created ?? false), success=\(hasSession), aliasedFrom=\(response.aliasedFrom ?? "none"), canonicalID=\(canonicalID?.uuidString ?? "same")")
            if hasSession {
                // Use the canonical id if the server redirected.
                let effectiveID = canonicalID ?? id
                if currentConversation == nil || currentConversation?.id != effectiveID {
            ...
            return EnsureResult(ok: false, canonicalID: nil, serverMessage: nil)
        }
    }
