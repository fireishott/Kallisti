...
final class ChatStore {
    private static let logger = Logger(subsystem: "net.fihonline.kallisti", category: "ChatStore")
    ...
    var heraldClient: any KallistiClientProtocol
    ...
    init(
        heraldClient: any KallistiClientProtocol,
    ...
    func submitNextEligible(for conversationID: UUID? = nil) async {
        guard var targetID = conversationID ?? conversation?.id else { return }
        ...
        let ensureResult = await heraldClient.ensureConversation(id: targetID)
        guard ensureResult.ok else {
            // T1.7: surface the server's typed message when one exists,
            // instead of the blanket connectivity copy.
            let error: String
            if let serverMsg = ensureResult.serverMessage, !serverMsg.isEmpty {
                error = serverMsg
            } else {
                error = "Could not reach the host to start a conversation. Check your connection and try again."
            }
            markOptimisticRowFailed(item, error: error)
            failOutboxItem(
                item,
        ...
        // T1.6: when the server redirected from a derived alias to the
        // canonical conversation id, remap all local state before
        // submitting — otherwise the send targets an id the server
        // doesn't recognize as the binding target.
        if let canonicalID = ensureResult.canonicalID, canonicalID != targetID {
            Logger.app.info("ensureConversation: server redirected \(targetID.uuidString) → \(canonicalID.uuidString)")
            adoptCanonicalConversationID(canonicalID, for: &item)
            ...
            for await update in stream {
                ...
                case .finished(let finalMessage, let usage, let diff, let context):
                    ...
                    if let idx = self.conversation?.messages.firstIndex(where: { $0.id == placeholderID }) {
                        ...
                        // 1) finalMessage.reasoning — set by LiveKallistiClient (SSE terminal
                        ...
                        // splitThinkingBlocks in LiveKallistiClient handles this on the sync
    ...
    // MARK: - T1.6: Canonical conversation id adoption

    /// Remap all local state from `item.conversationID` to `canonicalID`
    /// after the server redirected from a derived v5 alias to the real
    /// binding. Without this, the send targets an id the server doesn't
    /// recognize as the binding target.
    private func adoptCanonicalConversationID(_ canonicalID: UUID, for item: inout ChatOutboxRecord) {
        let oldID = item.conversationID
        // Update the outbox item's conversation id.
        item.conversationID = canonicalID
        // Update the conversation object if it matches.
        if conversation?.id == oldID {
            conversation?.id = canonicalID
        }
        // Update active leases key.
        if activeLeases[oldID] != nil {
            activeLeases[canonicalID] = activeLeases.removeValue(forKey: oldID)
        }
        Logger.app.info("adoptCanonicalConversationID: \(oldID.uuidString) → \(canonicalID.uuidString)")
    }
}
