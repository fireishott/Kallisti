...
final class SessionListStore {
    ...
    private let heraldClient: any KallistiClientProtocol
    ...
    init(heraldClient: any KallistiClientProtocol, chatStore: ChatStore, settingsStore: SettingsStore, persistence: any AppPersistenceStoreProtocol) {
    ...
    func createNewSession(title: String = "New Chat") async {
        do {
            ...
            let ensureResult = await heraldClient.ensureConversation(id: session.id)
            if !ensureResult.ok {
