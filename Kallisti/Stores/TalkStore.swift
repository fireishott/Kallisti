...
final class TalkStore {
    ...
    /// Kallisti 0.1.0: whether the host speech worker is configured.
    /// Set by the readiness closure in AppContainer.
    @ObservationIgnored var hostSpeechConfigured = false
    ...
    func refreshReadiness() async {
        ...
        // Kallisti 0.1.0: gate on server-side readiness (hermes-native speech worker).
        if let provider = talkReadinessProvider {
            let (ready, reason) = await provider()
            if !ready {
                canStartSession = false
                blockedReason = reason ?? "Talk is not ready on the host."
                return
            }
        }
    ...
    func startSession() async {
        ...
        // Gate on server-side readiness so a failed start surfaces
