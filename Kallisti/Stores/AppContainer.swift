...
final class AppContainer {
    ...
    let canvasStore: KallistiCanvasStore
    ...
    init(
        ...
        canvasStore: KallistiCanvasStore? = nil,
        ...
        self.canvasStore = canvasStore ?? KallistiCanvasStore()
    ...
    static func makeDefault(
        ...
        let heraldClient: any KallistiClientProtocol
        if usesMockPairingService {
            heraldClient = MockKallistiClient()
        } else {
            let liveClient = LiveKallistiClient(
                apiClient: apiClient,
                accessTokenProvider: { await sessionStore.currentAccessToken() },
                accessTokenRefresher: {
                    await sessionStore.refreshAccessTokenIfNeeded()
        ...
        let container = AppContainer(
            ...
            talkStore: {
                let ts = TalkStore()
                // Kallisti 0.1.0: no device key — host speech is config-driven.
                let relayBaseURLProvider: @MainActor () -> URL? = {
                ...
                let talkSpeechService = TalkSpeechService(
                    relayBaseURLProvider: relayBaseURLProvider,
                ...
                let tts = FallbackTTSService(
                    primary: talkSpeechService,
                    fallback: appleTTSService,
                    hostAvailableProvider: { ts.hostSpeechConfigured }
                )
                ts.ttsService = tts
                ...
                // Gate Talk on connector-side readiness (hermes-native speech worker).
                    ...
                    do {
                        ...
                        guard let http = response as? HTTPURLResponse,
                            ...
                            if statusCode == 401 || statusCode == 403 {
                                return (ready: false, blockedReason: "Host rejected the request — re-pair with the host.")
                            }
                            return (ready: false, blockedReason: "Talk readiness request failed.")
                        }
                        let resp = try TalkReadinessResponse.parse(data)
                        ts.hostSpeechConfigured = resp.configured ?? resp.ready
                        return (
                            ready: resp.ready,
                            blockedReason: resp.blockedReason
                                ?? (resp.ready ? nil : "Talk is not ready on the host.")
                        )
                    } catch {
                        return (
                            ready: false,
                ...
                if !usesMockPairingService {
                    ...
                    let asr = TalkTranscriptionService(
                        relayBaseURLProvider: relayBaseURLProvider,
                        accessTokenProvider: { [sessionStore] in
                            await sessionStore.currentAccessToken()
                        }
