...
/// Generic CodingKey for migrating away deprecated fields.
...
struct UserSettings: Codable, Hashable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // MiMo settings migrated away — decode and discard for backward compat
        // Using a nested container to avoid CodingKeys type mismatch
        if let nested = try? decoder.container(keyedBy: _AnyCodingKey.self) {
            _ = try? nested.decodeIfPresent(String.self, forKey: _AnyCodingKey("mimoTTSModel"))
            _ = try? nested.decodeIfPresent(String.self, forKey: _AnyCodingKey("mimoVoiceStyle"))
        }
