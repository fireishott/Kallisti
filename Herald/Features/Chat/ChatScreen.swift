...
struct ChatScreen: View {
        ...
        scrollAnchored {
            ...
            VStack(spacing: 0) {
                ...
                if settingsStore.settings.chatDisplayMode == .rich {
                    ...
                    onQueueNext: {
                        // Build 135.7: steer = interrupt + send. The steer
                        // button used to just enqueue behind the active turn
                        // ("sends this message after the current reply"),
                        // which is why steering during a long tool call "did
                        ...
                        if chatStore.isStreaming || chatStore.isServerTurnActive {
                            let record = chatStore.interruptAndSend(frozenText, attachments: frozenAttachments)
    ...
    private func sendMessage() {
        ...
        Task {
            ...
            // Build 135.7: if a turn is active, sending a message is a STEER -
            // interrupt the running turn and submit immediately instead of
            // parking behind the active lease (the old behavior queued behind
            // a multi-minute tool call and "steer did nothing"). When no turn
            // is active this is the normal two-phase send.
            if chatStore.isStreaming || chatStore.isServerTurnActive {
                let record = chatStore.interruptAndSend(
                    content,
                    attachments: attachments,
                    clientMessageID: clientMessageID
                )
                guard record != nil else {
                    // Rejected (empty/duplicate/over-limit) — keep the draft.
                    return
                }
                if let cid = conversationID { chatStore.clearDraft(for: cid) }
                pendingAttachments = []
