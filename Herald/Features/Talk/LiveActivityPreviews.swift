...
#Preview("Lock Screen — Tool Call") {
    VStack(spacing: 20) {
        LockScreenPreview(status: "Working on that...", toolName: "kallisti_delegate", elapsedSeconds: 45)
    }
    .padding()
    .background(Color(.systemGroupedBackground))
}
...
#Preview("Dynamic Island — All States") {
    VStack(spacing: 24) {
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            DynamicIslandExpandedPreview(status: "Working on that...", toolName: "kallisti_delegate")
                .frame(width: 360)
        }
