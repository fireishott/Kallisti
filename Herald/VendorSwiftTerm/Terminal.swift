 ...
 */
@MainActor public protocol TerminalDelegate: AnyObject {
    /**
     ...
     */
    func setForegroundColor (source: Terminal, color: TerminalColor)
    
    /**
     * The view should try to set the background color to the provided color
     */
    func setBackgroundColor (source: Terminal, color: TerminalColor)
    
    /**
     * The view should try to set the cursor color to the provided color.   If color is nil, the view can use a default.
     */
    func setCursorColor (source: Terminal, color: TerminalColor?)
    
    /**
     * This should return the current foreground and background colors to
     * report.
     */
    func getColors (source: Terminal) -> (foreground: TerminalColor, background: TerminalColor)
...
open class Terminal {
    ...
    nonisolated(unsafe) private static let keyboardModeStackLimit = 16
    ...
    var installedColors: [TerminalColor]
    // The blueprint for the colors, computed based on the installed colors
    var defaultAnsiColors: [TerminalColor]
    ...
    public var foregroundColor: TerminalColor = TerminalColor.defaultForeground {
        didSet {
            if settingFgColor {
                return
            }
    ...
    public var backgroundColor: TerminalColor = TerminalColor.defaultBackground {
        didSet {
            if settingBgColor {
                return
            }
    ...
    public var cursorColor: TerminalColor? = nil {
        didSet {
    ...
    {
        installedColors = TerminalColor.terminalAppColors
        defaultAnsiColors = TerminalColor.setupDefaultAnsiColors(initialColors: installedColors,
    ...
    public func installPalette (colors: [TerminalColor])
    {
        if colors.count != 16 {
            return
        }
    ...
    @MainActor class DECRQSS : @preconcurrency DcsHandler {
        var data: [UInt8]
...
//    }
    ...
    func oscChangeOrQueryColorIndex (_ data: ArraySlice<UInt8>)
    {
        ...
        while parsePos <= data.endIndex {
            ...
            if let newColor = TerminalColor.parseColor (data [parsePos..<end]) {
    ...
    func reportColor (oscCode: Int, color: TerminalColor) {
    ...
    func oscSetColors (_ data: ArraySlice<UInt8>, startAt: Int)
    {
        ...
        for (offset, text) in groups.enumerated() {
            ...
            guard let color = TerminalColor.parseColor(text) else {
                continue
            }
            switch target {
            case 0:
    ...
    func oscSetTextBackground (_ data: ArraySlice<UInt8>)
    {
        ...
        if let background = TerminalColor.parseColor(data) {
    ...
    {
        if let cursorColor = TerminalColor.parseColor(data) {
    ...
    nonisolated(unsafe) private static let allBidiStateProperties: Set<BidiStateProperty> = [
    ...
    private func cmdCharAttributes(_ pars: [Int]) {
        ...
        func parseExtendedColor(startIndex: Int, usesColon: Bool) -> (Attribute.TerminalColor?, Int) {
            ...
            if usesColon {
                ...
                case 2:
                    if chainLength >= 5 {
                        ...
                        return (Attribute.TerminalColor.trueColor(red: UInt8(min(red, 255)),
                ...
                case 5:
                    if chainLength >= 2 {
                        ...
                        return (Attribute.TerminalColor.ansi256(code: UInt8(min(255, code))), chainLength)
                    }
                default:
                    break
                }
                ...
                case 5:
                    if startIndex + 1 < pars.count {
                        let code = pars[startIndex + 1]
                        return (Attribute.TerminalColor.ansi256(code: UInt8(min(255, code))), 2)
                    }
                default:
                    break
                }
        ...
        func parseExtendedColor () -> Attribute.TerminalColor? {
        ...
        while i < parCount {
            ...
            case 30...37:
                // fg color 8
                fg = Attribute.TerminalColor.ansi256(code: UInt8(p - 30))
            case 38:
                i += 1
            ...
            case 40...47:
                // bg color 8
                bg = Attribute.TerminalColor.ansi256(code: UInt8(p - 40))
            case 48:
                i += 1
                if let parsed = parseExtendedColor() {
                    bg = parsed
            ...
            case 90...97:
                // fg color 16
                p += 8
                fg = Attribute.TerminalColor.ansi256(code: UInt8(p - 90))
            case 100...107:
                // bg color 16
                p += 8;
                bg = Attribute.TerminalColor.ansi256(code: UInt8(p - 100))
...
//                break
    ...
    nonisolated(unsafe) static var matchColorCache : [Int:Int] = [:]
    ...
    nonisolated(unsafe) private static let ghosttyImplicitLinkRegex: NSRegularExpression? = {
    ...
    nonisolated(unsafe) private static let ghosttyContinuationCharacters = CharacterSet(
