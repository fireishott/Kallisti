...
nonisolated(unsafe) private let fontKeyNS = NSAttributedString.Key.font.rawValue as NSString
nonisolated(unsafe) private let foregroundKeyNS = NSAttributedString.Key.foregroundColor.rawValue as NSString
nonisolated(unsafe) private let backgroundKeyNS = NSAttributedString.Key.backgroundColor.rawValue as NSString
nonisolated(unsafe) private let selectionBackgroundKeyNS = NSAttributedString.Key.selectionBackgroundColor.rawValue as NSString
nonisolated(unsafe) private let underlineStyleKeyNS = NSAttributedString.Key.underlineStyle.rawValue as NSString
nonisolated(unsafe) private let strikethroughStyleKeyNS = NSAttributedString.Key.strikethroughStyle.rawValue as NSString
...
nonisolated(unsafe) private var cgColorCache: [TTColor: CGColor] = [:]
...
nonisolated(unsafe) private var fallbackFontCache: [FallbackFontKey: TTFont] = [:]
...
nonisolated(unsafe) private var ctLineCache: [NSAttributedString: CTLine] = [:]
...
struct GlyphSlotFit {
    ...
    nonisolated(unsafe) static let identity = GlyphSlotFit()
...
extension TerminalView {
    ...
    func mapColor (color: Attribute.TerminalColor, isFg: Bool, isBold: Bool, useBrightColors: Bool = true) -> TTColor
    {
        switch color {
        case .defaultColor:
            if isFg {
    ...
    public func installColors (_ colors: [TerminalColor])
    {
    ...
    public func setBackgroundColor(source: Terminal, color: TerminalColor) {
        // Can not implement this until I change the color to not be this struct
        nativeBackgroundColor = TTColor.make (color: color)
        colorsChanged()
    }
    
    public func setForegroundColor(source: Terminal, color: TerminalColor) {
    ...
    public func setCursorColor(source: Terminal, color: TerminalColor?, textColor: TerminalColor?) {
        ...
        while true {
            ...
            // U+2580...U+259F
                ...
                if !blinkHidden && UnicodeUtil.prefersTextPresentation(renderedCharacter) {
                    // Steer font fallback away from Apple TerminalColor Emoji for
