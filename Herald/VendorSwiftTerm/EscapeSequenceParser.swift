...
public class EscapeSequenceParser {
#if canImport(os)
    nonisolated(unsafe) private static let profileLog = OSLog(subsystem: "org.tirania.SwiftTerm", category: "ParserProfile")
    nonisolated(unsafe) private static let profileEnabled = ProcessInfo.processInfo.environment["SWIFTTERM_PROFILE"] == "1"
#endif
    ...
    nonisolated(unsafe) static let NonAsciiPrintable : UInt8 = 0xa0
    ...
    private static var sharedVt500Table: TransitionTable = EscapeSequenceParser.buildVt500TransitionTable()

    /// CSI and DCS parameter values use the same 16-bit saturating range as Ghostty.
    nonisolated(unsafe) static let maximumParameterValue = Int(UInt16.max)

    /// Sequences beyond this limit are dropped instead of growing parser state without bound.
    nonisolated(unsafe) static let maximumParameterCount = 24
