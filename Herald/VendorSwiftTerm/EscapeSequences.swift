...
struct ControlCodes  {
    nonisolated(unsafe) static let NUL : UInt8 = 0x00
    nonisolated(unsafe) static let BEL : UInt8 = 0x07
    nonisolated(unsafe) static let BS  : UInt8 = 0x08
    nonisolated(unsafe) static let HT  : UInt8 = 0x09
    nonisolated(unsafe) static let LF  : UInt8 = 0x0a
    nonisolated(unsafe) static let VT  : UInt8 = 0x0b
    nonisolated(unsafe) static let FF  : UInt8 = 0x0c
    nonisolated(unsafe) static let CR  : UInt8 = 0x0d
    nonisolated(unsafe) static let SO  : UInt8 = 0x0e
    nonisolated(unsafe) static let SI  : UInt8 = 0x0f
    nonisolated(unsafe) static let CAN : UInt8 = 0x18
    nonisolated(unsafe) static let SUB : UInt8 = 0x1a
    nonisolated(unsafe) static let ESC : UInt8 = 0x1b
    nonisolated(unsafe) static let SP  : UInt8 = 0x20
    nonisolated(unsafe) static let DEL : UInt8 = 0x7f
}
...
public struct EscapeSequences {
    ...
    nonisolated(unsafe) public static let cmdNewLine: [UInt8] = [ 10 ]
    /// Sent when the user pressed the return character
    nonisolated(unsafe) public static let cmdRet: [UInt8] = [ 13 ]
    /// Sent when the user pressed the escape key
    nonisolated(unsafe) public static let cmdEsc: [UInt8] = [ 0x1b ]
    /// Sent when the user pressed the backspace key (Delete on the Mac keyboard)
    nonisolated(unsafe) public static let cmdDel: [UInt8] = [ 0x7f ]
    /// Send when the user pressed the delete key (Delete pointing to the right on Mac keyboards)
    nonisolated(unsafe) public static let cmdDelKey: [UInt8] = [ 0x1b, 0x5b, 0x33, 0x7e ]
    /// Send in application cursor mode when the user presses the up arrow key
    nonisolated(unsafe) public static let moveUpApp: [UInt8] = [ 0x1b, 0x4f, 0x41 ]
    /// Send in normal mode when the user presses the up arrow key
    nonisolated(unsafe) public static let moveUpNormal: [UInt8] = [ 0x1b, 0x5b, 0x41 ]
    /// Send in application cursor mode when the user presses the down arrow key
    nonisolated(unsafe) public static let moveDownApp: [UInt8] = [ 0x1b, 0x4f, 0x42 ]
    /// Send in normal mode when the user presses the down arrow key
    nonisolated(unsafe) public static let moveDownNormal: [UInt8] = [ 0x1b, 0x5b, 0x42 ]
    /// Send in application cursor mode when the user presses the left arrow key
    nonisolated(unsafe) public static let moveLeftApp: [UInt8] = [ 0x1b, 0x4f, 0x44 ]
    /// Send in normal mode when the user presses the left arrow key
    nonisolated(unsafe) public static let moveLeftNormal: [UInt8] = [ 0x1b, 0x5b, 0x44 ]
    /// Send in application cursor mode when the user presses the right arrow key
    nonisolated(unsafe) public static let moveRightApp: [UInt8] = [ 0x1b, 0x4f, 0x43 ]
    /// Send in normal mode when the user presses the right arrow key
    nonisolated(unsafe) public static let moveRightNormal: [UInt8] = [ 0x1b, 0x5b, 0x43 ]
    /// Send in application cursor mode when the user presses the home key
    nonisolated(unsafe) public static let moveHomeApp: [UInt8] = [ 0x1b, 0x4f, 0x48 ]
    /// Send in normal mode when the user presses the home key
    nonisolated(unsafe) public static let moveHomeNormal: [UInt8] = [ 0x1b, 0x5b, 0x48 ]
    /// Send in application cursor mode when the user presses the end key
    nonisolated(unsafe) public static let moveEndApp: [UInt8] = [ 0x1b, 0x4f, 0x46 ]
    /// Send in normal mode when the user presses the end key
    nonisolated(unsafe) public static let moveEndNormal: [UInt8] = [ 0x1b, 0x5b, 0x46 ]
    /// Send in normal mode when the user presses the tab key
    nonisolated(unsafe) public static let cmdTab: [UInt8] = [ 9 ]
    /// Send in normal mode when the user presses shift-tab key
    nonisolated(unsafe) public static let cmdBackTab: [UInt8] = [ 0x1b, 0x5b, 0x5a ]
    /// Send in normal mode when the user presses the page up key
    nonisolated(unsafe) public static let cmdPageUp: [UInt8] = [ 0x1b, 0x5b, 0x35, 0x7e ]
    /// Send in normal mode when the user presses the page down key
    nonisolated(unsafe) public static let cmdPageDown: [UInt8] = [ 0x1b, 0x5b, 0x36, 0x7e ]
    /// Sends the insert key
    nonisolated(unsafe) public static let cmdInsert: [UInt8] = [ 0x1b, 0x5b, 0x32, 0x7e ]
    
    /// Emacs back-word (meta-b)
    nonisolated(unsafe) public static let emacsBack: [UInt8] = [ 0x1b, 0x62]
    /// Emacs forward-word (meta-f)
    nonisolated(unsafe) public static let emacsForward: [UInt8] = [ 0x1b, 0x66]

    /// Control-left arrow
    nonisolated(unsafe) public static let controlLeft: [UInt8] = [ 0x1b, 0x5b, 0x31, 0x3b, 0x35, 0x44 ]
    /// Control-right arrow
    nonisolated(unsafe) public static let controlRight: [UInt8] = [ 0x1b, 0x5b, 0x31, 0x3b, 0x35, 0x43 ]
    
    /// Beginning of pasted text when bracketed-paste is enabled (mode 2004)
    /// The sequence is `ESC [ 200 ~`
    nonisolated(unsafe) public static var bracketedPasteStart: [UInt8] = [0x1b, 0x5b, 0x32, 0x30, 0x30, 0x7e]
    
    /// End of pasted text when bracketed-paste is enabled (mode 2004)
    /// /// The sequence is `ESC [ 201 ~`
    nonisolated(unsafe) public static var bracketedPasteEnd: [UInt8] = [0x1b, 0x5b, 0x32, 0x30, 0x31, 0x7e]
    ...
    nonisolated(unsafe) public static let cmdF: [[UInt8]] = [
         [ 0x1b, 0x4f, 0x50 ], /* F1 */
         [ 0x1b, 0x4f, 0x51 ], /* F2 */
         [ 0x1b, 0x4f, 0x52 ], /* F3 */
         [ 0x1b, 0x4f, 0x53 ], /* F4 */
