...
extension NSColor {
    nonisolated(unsafe) private static let srgbColorSpace = CGColorSpace(name: CGColorSpace.sRGB)!

    func getTerminalColor () -> TerminalColor {
        guard let color = self.usingColorSpace(.sRGB) else {
            return TerminalColor.defaultForeground
        }
        ...
        return TerminalColor(red: UInt16(clamp(red)*65535), green: UInt16(clamp(green)*65535), blue: UInt16(clamp(blue)*65535))
    }
    ...
    static func make (color: TerminalColor) -> NSColor
    {
